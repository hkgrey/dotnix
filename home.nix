...
# in
{
  ...
  programs.bash = {
    ...
    initExtra = builtins.readFile ./bashrc + ''
      ...
      # aws-sso-cli shell integration (declarative equivalent of
      # `aws-sso setup completions`). Placed in initExtra so it loads after the
      # bash-completion helpers (_get_comp_words_by_ref, __ltrim_colon_completions)
      # sourced in bashrcExtra.
      __aws_sso_profile_complete() {
          ...
          COMPREPLY=($(compgen -W "$(${pkgsUnstable.aws-sso-cli}/bin/aws-sso $_args list --csv -P "Profile=$cur" Profile)" -- ""))
      ...
      aws-sso-profile() {
          ...
          if [ -n "$_sso" ]; then
              eval $(${pkgsUnstable.aws-sso-cli}/bin/aws-sso $_args -S "$_sso" eval -p "$_profile")
          else
              eval $(${pkgsUnstable.aws-sso-cli}/bin/aws-sso $_args eval -p "$_profile")
          fi
      ...
      aws-sso-clear() {
          ...
          eval $(${pkgsUnstable.aws-sso-cli}/bin/aws-sso eval $_args -c)
      }

      complete -F __aws_sso_profile_complete aws-sso-profile
      complete -C ${pkgsUnstable.aws-sso-cli}/bin/aws-sso aws-sso
    '';
